    ...
    _this._npmInstall(program, codeDirectory, function (err) {
      ...
      archive(program, codeDirectory, function (err, buffer) {
        ...
        async.map(regions, function (region, cb) {
          ...
          var listFunctionParams = {
             MaxItems: MAX_FUNCTIONS_COUNT
          }
          var listVersionParams = {
            FunctionName: program.functionName
          }
          var updatedCodeParams = {
             FunctionName: program.functionName,
             ZipFile: buffer
          }
          var runStat = {
          ...
          async.series([
            ...
            function(done) {
               if(runStat.NameExists) {
                  ...
                  if(runStat.Versions.indexOf(program.version) == -1) {
                     console.log("=> Version "+program.version+" does not exist. Updating code in $LATEST and publishing...")
                     ...
                     cb("Function Already deployed and version "+program.version+" already exists!",null)
                  }
               } else {

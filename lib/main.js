			...
			archive(program, codeDirectory, function (err, buffer) {
				...
				function(done) {
					if(runStat.NameExists) {
						...
						if(runStat.Versions.indexOf(program.functionVersion) == -1) {
							...
							done("Function already deployed and version "+program.functionVersion+" already exists!")
						}
					} else {
				...
				function(err,results) {
					if(err) {
						...
						console.log("=> Done uploading function " + program.functionName + " as version $LATEST");
					}
				});
			});
		});
